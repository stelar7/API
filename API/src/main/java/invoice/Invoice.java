...
public class Invoice
{
	...
	public void save(String filename)
	{
		...
		try
		{
			...
			for (InvoiceItem item : getItems())
			{
				...
				float yOffset = 580 - skipSize * iterationCount++;
			...
			stream.showText("Ved forsinket betaling kan det påløpe purregebyr etter statens satser. (65kr per 2015)");
			stream.endText();
	...
	{
		int[] total = {0}; // Disgusting "objects in lambda needs to be final" hack...
		items.forEach(i -> total[0] += i.getItemCount());
		return total[0];
	}
	...
	{
		MonetaryAmount[] total = {Money.of(0, "NOK")};// Disgusting "objects in lambda needs to be final" hack...
