...
import div.osu.Beatmap.*;
import div.osu.Beatmap.HitObject.*;
import div.osu.Beatmap.HitObject.Slider.*;
...
public class OsuParser
{
    ...
    public static class CurveCalc
    {
        ...
        private static Vector2 getEndPoint(final CurveType curve, final float sliderLength, final List<Vector2> points)
        {
            switch (curve)
            {
                ...
                case BEZIER:
                {
                    ...
                    final Bezier<Vector2> bezier = new Bezier<Vector2>(useme.toArray(new Vector2[useme.size()]));
                ...
                case PASS_THROUGH:
                    ...
                    float radians = sliderLength / circir.radius;
                    ...
                    return CurveCalc.rotate(circir.x, circir.y, a.x, a.y, radians);
        ...
        public static Vector2 rotate(final float cx, final float cy, final float x, final float y, final float radians)
        {
            final float cos = (float) Math.cos(radians);
            final float sin = (float) Math.sin(radians);

            return new Vector2(((cos * (x - cx)) - (sin * (y - cy))) + cx, (sin * (x - cx)) + (cos * (y - cy)) + cy);
        }
    }
    ...
    private static List<String>  timingLines     = new ArrayList<>();
    private static List<String>  eventLines      = new ArrayList<>();
        ...
        switch (OsuParser.sectionString)
        {
            ...
            default:
            {
                if (OsuParser.sectionString.isEmpty())
                {
                    ...
                    if (matcher.matches())
                    {
                        OsuParser.beatmap.fileFormat = matcher.group();
                        return;
                    }
                }
    ...
    public static Beatmap parseOsuSong(final Path f)
    {
        try
        {
